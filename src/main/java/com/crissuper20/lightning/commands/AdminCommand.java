...
public class AdminCommand implements CommandExecutor, TabCompleter {
    ...
    private boolean handleStats(CommandSender sender) {
        ...
        sender.sendMessage("§eBackend: §f" + plugin.getLnService().getBackendName());
        ...
        InvoiceMonitor monitor = plugin.getInvoiceMonitor();
        if (monitor != null) {
            sender.sendMessage("");
            sender.sendMessage("§6Invoice Monitor:");
        ...
        if (player == null) {
            ...
            CompletableFuture<String> future = walletManager.fetchBalanceFromLNbits(player)
                .thenApply(balance -> {
                    ...
                    String name = Bukkit.getOfflinePlayer(uuid).getName(); // Name lookup is slow
    ...
    CompletableFuture<Void> allFutures = CompletableFuture.allOf(balanceFutures.toArray(new CompletableFuture[0]));
    ...
    allFutures.thenRun(() -> {
            ...
            for (CompletableFuture<String> future : balanceFutures) {
                // Since allFutures already waited for these, .join() here is a safe, fast retrieve.
    ...
    private boolean handleInvoices(CommandSender sender) {
        ...
        if (monitor == null) {
            sender.sendMessage("§cInvoice monitor not initialized.");
            return true;
        }
        
        Map<String, InvoiceMonitor.PendingInvoice> pending = monitor.getAllPendingInvoices();
    ...
    private boolean handleReload(CommandSender sender) {
        ...
        try {
            ...
            sender.sendMessage("§7Note: Some changes may require a restart to take effect.");
        } catch (Exception e) {
    ...
    public List<String> onTabComplete(CommandSender sender, Command command, String alias, String[] args) {
        ...
        if (args.length == 1) {
            return Arrays.asList("stats", "wallets", "sync", "invoices", "health", "metrics", "reload");
        }
